...
def nfsmw_model_input(guessed_input):
    # Joystick goes from 0 to 32'000
    joystick_amp = 50          # default should be 16'000?
    
    vj.open()
    xPos = (guessed_input[3] - guessed_input[1]) * joystick_amp
    if guessed_input[0] > -10:
        button = 1
    elif guessed_input[2] > 0:
        button = 3
    else:
        button = 0

    joystickPosition = vj.generateJoystickPosition(wAxisX = int(16000+xPos),
                                                       lButtons=button)
    vj.update(joystickPosition)
    vj.close()


def nfsmw_model1_continuous_input(guessed_input):
    forwards = guessed_input[0]
    left = guessed_input[1]
    backwards = guessed_input[2]
    right = guessed_input[3]
    
    JOYSTICK_RANGE = 32786 # Joystick goes from 0 to 32'000
    JOYSTICK_MIDDLE = JOYSTICK_RANGE / 2
    LEFT_RIGHT_AMP = 50
    PEDAL_AMP = 1500
    
    vj.open()
    # steering wheel
    xPos = ((right - left) * LEFT_RIGHT_AMP) + JOYSTICK_MIDDLE
    # break pedal
    yPos = max(backwards - forwards, 0) * PEDAL_AMP
    # gas pedal
    zPos = max(forwards - backwards, 0) * PEDAL_AMP

    joystickPosition = vj.generateJoystickPosition(wAxisX = int(xPos),
                                                   wAxisY = int(yPos),
                                                   wAxisZ = int(zPos))
    vj.update(joystickPosition)
    vj.close()
...
def nfsmw_speed_direction_input(guessed_input):
    # Joystick goes from 0 to 32'000
    joystick_range = 32786
    joystick_amp = joystick_range/2 * 1
    
    vj.open()
    xPos = -guessed_input[1] * joystick_amp
    zPos = guessed_input[0] * joystick_amp

    joystickPosition = vj.generateJoystickPosition(wAxisX = int(joystick_range/2 + xPos),
                                                   wAxisZ = int(joystick_range/2 + zPos))
    vj.update(joystickPosition)
    vj.close()
...
def test():
    ...
    for i in range(0,1000,1):
        ...
        joystickPosition = vj.generateJoystickPosition(wAxisX = 16000+xPos,
                                                       wAxisY = 16000+yPos,
                                                       lButtons=1)
        vj.update(joystickPosition)
        time.sleep( 0.01 )
